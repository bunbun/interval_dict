...
/* Using first(), last(), lower(), upper() from boost::icl */
...
namespace interval_dict
{
  ...
  // We need to have a sorted array of both interval begins and ends, tagged
  ...
  //    In theory, only update if (beg_index==0), i.e. ends. However, if we
  //    ignore updates for starts, we save a conditional
  template<typename Interval>
  class CountOverlap
  {
    public:
    /// @return The number of subsequent intervals each interval overlaps with.
    /// @brief Uses first and last to accurate calculate overlaps
  ...
  struct has_icl_first : std::false_type
  {
  };
  template<typename T>
  ...
  template<typename, typename = void>
  struct has_icl_last : std::false_type
  {
  };
  template<typename T>
  struct has_icl_last<
