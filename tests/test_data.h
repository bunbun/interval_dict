...
// aa     0----0 0-------------------------------------------0 bb 1---------1
// 1----1 bb                         2---------2 cc 3-------------------3 cc
// 3-----------------------------3 dd                         5---------5
// 5----------------------------5 dd                                   6----6 dd
// 7----7 Query        q--------------------------------------q fill to start
// <-|10 fill to end                                       50|-> fill_gaps(3)
...
namespace test_detail
{
  ...
  initial (const std::vector<typename Interval::domain_type> &values)
  {
    ...
    return {
      ...
      {"dd"s,
       6,
       boost::icl::left_subtract (Interval {values[5], values[7]},
  ...
  template<typename Interval>
  std::vector<std::tuple<int, double, Interval>>
  initial2 (const std::vector<typename Interval::domain_type> &values)
  {
    using namespace std::string_literals;
    return {
  ...
  template<typename Interval>
  std::vector<std::tuple<std::string, int, Interval>>
  intervals (const std::vector<typename Interval::domain_type> &values)
  {
    using namespace std::string_literals;
  ...
  template<typename Interval>
  std::vector<std::tuple<std::string, std::vector<int>, Interval>>
  disjoint_intervals (const std::vector<typename Interval::domain_type> &values)
  {
    using namespace std::string_literals;
    const auto adjust = Adjust<Interval> {};
    return {
  ...
  template<typename Interval>
  std::string to_str (Interval interval)
  {
  ...
  template<typename Interval>
  std::string to_str (const std::vector<typename Interval::domain_type> &values)
  {
    using namespace std::string_literals;
    const auto adjust = Adjust<Interval> {};
    auto result = "aa\t[0]\t"s + to_str (Interval {values[0], values[1]})
                  + "\n"
                    "aa\t[0]\t"s
                  + to_str (Interval {values[8], values[17]})
                  + "\n"
                    "bb\t[1]\t"s
                  + to_str (Interval {values[1], values[3]})
                  + "\n"
                    "bb\t[1, 2]\t"s
                  + to_str (Interval {values[4], values[5]})
                  + "\n"
                    "bb\t[2]\t"s
                  + to_str (adjust.lower ({values[5], values[6]}))
                  + "\n"
                    "cc\t[3]\t"s
                  + to_str (Interval {values[3], values[11]})
                  + "\n"
                    "dd\t[5]\t"s
                  + to_str (Interval {values[4], values[6]})
                  + "\n"
                    "dd\t[6, 7]\t"s
                  + to_str (adjust.lower ({values[6], values[7]}))
                  + "\n"
                    "dd\t[5]\t"s
                  + to_str (Interval {values[9], values[15]}) + "\n";
...
template<>
struct TestDataValues<boost::gregorian::date>
{
...
template<>
struct TestDataValues<boost::posix_time::ptime>
{
...
struct TestData
{
  ...
  typename interval_dict::IntervalTraits<Interval>::BaseDifferenceType
    test_duration
