                    ...
                    boost::icl::discrete_interval<boost::gregorian::date>)
{
  ...
  GIVEN ("An IntervalDict with overlapping intervals")
  {
    ...
    const auto pairs = [&] () -> std::vector<std::pair<Key, Value>>
    {
      std::set<std::pair<Key, Value>> pairs;
    ...
    const auto inverse_pairs = [&] () -> std::vector<std::pair<Value, Key>>
    {
      std::set<std::pair<Value, Key>> pairs;
      for (const auto &[key, value, _] : import_data)
      {
