...
TEMPLATE_TEST_CASE ("Test inverse_find() for different interval types",
                    "[find]",
                    ...
                    boost::icl::discrete_interval<boost::gregorian::date>)
{
  ...
  GIVEN ("An IntervalDict with overlapping intervals")
  {
    ...
    WHEN ("Calling find() the largest extent")
    {
      ...
      {
        REQUIRE (inverse_dict.inverse_find (all_keys, max_ext)
    ...
    WHEN ("Calling find() with a query extent")
    {
            ...
            "that interval")
      {
        ...
        REQUIRE (inverse_dict.inverse_find ("aa"s, adjust.both (query))
                 == std::vector<Value> ());
        // not a key
        REQUIRE (inverse_dict.inverse_find ("not a key"s, query)
                 == std::vector<Value> ());
      }
    }
    ...
    WHEN ("Calling find() with an empty query extent")
    {
      THEN ("Expect all empty results")
      {
        ...
        REQUIRE (inverse_dict.inverse_find ("bb"s,
                 ...
                 == std::vector<Value> {});
        REQUIRE (
          inverse_dict.inverse_find ("bb"s, boost::icl::upper (query_max))
          == std::vector<Value> {});
      }
    }
  }
