                    ...
                    boost::icl::discrete_interval<boost::gregorian::date>)
{
  ...
  GIVEN ("An IntervalDict with overlapping intervals")
  {
    ...
    const auto query_max
      = Interval {interval_dict::IntervalTraits<Interval>::maximum (),
                  interval_dict::IntervalTraits<Interval>::maximum ()};
    ...
    WHEN ("Calling find() with a query extent")
    {
            ...
            "that interval")
      {
        ...
        REQUIRE (test_dict.find ("aa"s, adjust.both (query))
                 == std::vector<Value> ());
        // not a key
        REQUIRE (test_dict.find ("not a key"s, query) == std::vector<Value> ());
      }
    }
    ...
    WHEN ("Calling find() with a single points")
    {
      THEN ("Expect all corresponding values for those keys")
      {
        REQUIRE (test_dict.find (
                   "bb"s, boost::icl::lower (query), boost::icl::upper (query))
                 == std::vector {1, 2});
        ...
        if (!boost::icl::is_empty (query_end))
        {
          REQUIRE (test_dict.find (all_keys, query_end)
                   == std::vector {1, 2, 3, 5});
          REQUIRE (test_dict.find ("bb"s, boost::icl::lower (query_end))
                   == std::vector {1, 2});
        }
      }
    }
    ...
    WHEN ("Calling find() with a multiple intervals")
    {
      THEN ("Expect all corresponding values for those keys")
      {
        ...
        // not a key
        REQUIRE (test_dict.find ("not a key"s, queries) == std::vector<Value> ());
        // Empty intervals
        REQUIRE (test_dict.find ("dd"s, interval_dict::Intervals<Interval> ())
                 == std::vector<Value> {});
        REQUIRE (
          test_dict.find ("not a key"s, interval_dict::Intervals<Interval> ())
          == std::vector<Value> {});
      }
    }
    ...
    WHEN ("Calling find() with an empty query extent")
    {
      THEN ("Expect all empty results")
      {
        ...
        if (boost::icl::is_empty (query_end))
        {
          REQUIRE (test_dict.find (all_keys, query_end) == std::vector<Value> {});
        }
        REQUIRE (test_dict.find (all_keys, query_max) == std::vector<Value> {});
        REQUIRE (test_dict.find ("bb"s, query_max) == std::vector<Value> {});
