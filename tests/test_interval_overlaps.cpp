...
class PermuteIntervals
{
  ...
  std::vector<std::tuple<int, int>> permute ()
  {
    ...
    for (const auto choice : m_state)
    {
      ...
      result.push_back ({reverse_indices.back (), reverse_indices[rev_choice]});
...
template<typename T>
void get_loci (std::vector<T> &loci, int count)
{
  T pos = 0;
...
std::vector<interval_dict::ValueInterval<Value, Interval>>
...
{
  std::vector<interval_dict::ValueInterval<Value, Interval>> m_intervals;
...
std::vector<int_fast32_t> manual_count_overlapping (
  const std::vector<interval_dict::ValueInterval<Value, Interval>> &m_intervals)
{
  const size_t count_intervals = m_intervals.size ();
  std::vector<int_fast32_t> overlap_counts (count_intervals, 0);
  for (size_t i = 0; i < count_intervals; ++i)
  {
    for (size_t j = i + 1; j < count_intervals; ++j)
    {
                    ...
                    boost::icl::discrete_interval<boost::gregorian::date>)
{
  ...
  while (1)
  {
    ...
    const auto m_intervals
      = make_intervals<Value, Interval> (pairs, interval_indices, loci);
                    ...
                    boost::icl::discrete_interval<boost::gregorian::date>)
{
  ...
  while (1)
  {
    ...
    for (int i = 0; i < COUNT_INTERVALS; ++i)
    {
      ...
      const auto bs
        = binary_search_touch (query_interval, m_intervals, m_max_right_edges);
      REQUIRE (ls == bs);
      max_count_overlaps = std::max (max_count_overlaps, ls.size ());
    }
    ++permutation_index;
