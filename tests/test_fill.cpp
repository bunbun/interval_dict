                   ...
                   boost::icl::discrete_interval<boost::gregorian::date>)
{
    ...
    GIVEN("An IntervalDict with overlapping intervals")
    {
        ...
        const auto MIN = interval_dict::IntervalTraits<Interval>::lowest();
            ...
            THEN("Some keys are extended to the start of time.")
            {
                ...
                REQUIRE(copy(test_dict).fill_to_start(test_pos, test_len) -
                        ...
                        IDict(ImportData{
                            {"aa",
                             0,
                             adjust.upper({values[0] - test_len, values[0]})},
                            {"bb",
                             1,
                             adjust.upper({values[1] - test_len, values[1]})},
                        }));
            }
        }
    }
...

{
    ...
    GIVEN("An IntervalDict with overlapping intervals")
    {
        ...
        const IDict test_dict(test_data.initial());
        const auto adjust = Adjust<Interval>{};
        const auto MAX = interval_dict::IntervalTraits<Interval>::max();
        const auto MAXSZ = interval_dict::IntervalTraits<Interval>::max_size();
        const auto all_keys = std::vector{"aa"s, "bb"s, "cc"s, "dd"s, "ee"s};
        const auto values = TestDataValues<BaseType>::values();
            ...
            THEN("Some keys are extended to the end of time.")
            {
                ...
                REQUIRE(copy(test_dict).fill_to_end(test_pos, test_len) -
                        ...
                        IDict(ImportData{
                            {"aa",
                             0,
                             adjust.lower({values[17], values[17] + test_len})},
                            {"cc",
                             3,
                             adjust.lower({values[11], values[11] + test_len})},
                            {"dd",
                             5,
                             adjust.lower({values[15], values[15] + test_len})},
                        }));
            }
        }
    }
            ...
            THEN("All gaps of interrupted associations are filled.")
            {
                ...
                REQUIRE((copy(test_dict).fill_gaps(test_len) - test_dict) ==
                        IDict(ImportData{
                            {"aa",
                             0,
                             adjust.lower({values[1], values[1] + test_len})},
                            {"aa",
                             0,
                             adjust.upper({values[8] - test_len, values[8]})},
                            {"bb", 1, adjust.both({values[3], values[4]})},
                        }));
            }
        }
            ...
            THEN("All gaps are filled.")
            {
                ...
                REQUIRE(
                    ...
                    IDict(ImportData{
                        {"dd",
                         5,
                         adjust.upper({values[9] - test_len, values[9]})},
                        {"dd",
                         6,
                         adjust.lower({values[7], values[7] + test_len})},
                        {"dd",
                         7,
                         adjust.lower({values[7], values[7] + test_len})},
                    }));
